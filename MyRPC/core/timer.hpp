...
class Timer {
 public:
  ...
  bool GetLastTimer(TimerData& timerData) {
    while (not timers_.empty()) {
      ...
      timers_.pop();
      if (cancel_ids_.find(timerData.id_) != cancel_ids_.end()) {  // 被取消的定时器不执行，直接删除
  ...
  int64_t TimeOutMs(TimerData& timerData) {
    int64_t temp = timerData.abs_time_ms_ + 1 - GetCurrentTimeMs();  // 多1ms，确保后续的定时器必定能超时
    if (temp > 0) {
      return temp;
    }
    return 0;
 ...
 private:
  bool isExpire(TimerData& timerData) { return GetCurrentTimeMs() >= timerData.abs_time_ms_; }
